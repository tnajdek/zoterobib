...
export function runSingle(label, cmd, args, env = {}) {
	return new Promise((resolve, reject) => {
		...
		const collect = data => buffer.push(...data.toString().split('\n').filter(l => l.trim().length));
		proc.stdout.on('data', collect);
		proc.stderr.on('data', collect);

		proc.on('close', code => {
			if (interval) clearInterval(interval);
			const prefix = INTERACTIVE ? '\r\x1b[2K' : '';
			if (code === 0) {
				process.stdout.write(`${prefix}  ${GREEN}✓${RESET} ${label}\n`);
				resolve();
			} else {
				process.stdout.write(`${prefix}  ${RED}✗${RESET} ${label}\n`);
				process.stdout.write(buffer.map(l => `${DIM}[${label}]${RESET} ${l}\n`).join(''));
